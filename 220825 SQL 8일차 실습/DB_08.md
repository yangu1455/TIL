...
# Movie.objects.create(director = director_.name, genre = genre_.title, title = '기생충', opening_date = '2019-05-30', running_time = 132, screening = False)
...
for m in movies:
    ...
    # NOT NULL constraint failed: db_movie.director_id
    # 이것때문에 3시간.. 날린것 같다! ^^
```
...
for i in Movie.objects.all():
    director_ = Director.objects.get(id = i.director_id)
    genre_ = Genre.objects.get(id = i.genre_id)

    print(director_, genre_, i.title, i.opening_date, i.running_time, i.screening, i.director_id, i.genre_id)
```
...
for i in Movie.objects.all():
    ...
    print(director_.name, genre_, i.title, i.opening_date, i.running_time, i.screening)
```
...
for i in Movie.objects.all():
    ...
    print(director_.name, genre_.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
director_ = Director.objects.get(id = M.director_id)
genre_ = Genre.objects.get(id = M.genre_id)

print(director_.name, genre_.title, M.title, M.opening_date, M.running_time, M.screening)
```
...
```python
n_bong = Director.objects.get(name = '봉준호')

for i in Movie.objects.filter(director = n_bong).order_by('opening_date'):
    director_ = Director.objects.get(id = i.director_id)
    genre_ = Genre.objects.get(id = i.genre_id)
...
i = Movie.objects.filter(director = n_bong).order_by('opening_date')[1]
...
```python
for i in Movie.objects.filter(opening_date__gte = '2022-01-01').order_by('opening_date'):
    director_ = Director.objects.get(id = i.director_id)
    genre_ = Genre.objects.get(id = i.genre_id)
...
```python
n_bong = Director.objects.get(name = '봉준호')
j_drama = Genre.objects.get(title = '드라마')

for i in Movie.objects.filter(director = n_bong, genre = j_drama).order_by('opening_date'):
    director_ = Director.objects.get(id = i.director_id)
    genre_ = Genre.objects.get(id = i.genre_id)
...
```python
bong_id = Director.objects.get(name = '봉준호').id

for i in Movie.objects.exclude(director = bong_id).order_by('opening_date'):
    director_ = Director.objects.get(id = i.director_id)
    genre_ = Genre.objects.get(id = i.genre_id)
